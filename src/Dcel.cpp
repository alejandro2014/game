...
void Dcel::ReverseFloat(unsigned int* inFloat) {
...
void Dcel::cargarDcelBmp(const char *nombreFichero) {
	...
	float xMaxima = -1.0;
	float xMinima = 10000000.0;
	float yMaxima = -1.0;
	float yMinima = 10000000.0;
	//-int desplArista;
	//-int desplBordes;
	...
	for(i = 0; i < alto; i++) {
		for(j = 0; j < ancho; j++) {
			...
			if(vertActual->x > xMaxima) xMaxima = vertActual->x;
			if(vertActual->x < xMinima) xMinima = vertActual->x;
			if(vertActual->y > yMaxima) yMaxima = vertActual->y;
			if(vertActual->y < yMinima) yMinima = vertActual->y;

			desplaz = (num_v + j - ancho*(i + 1)) + 54;
			altura = (GLfloat)*(ficheroBruto + desplaz);
			vertActual->z = altura/256;
		}
	}
cout << "5" << endl;
	puntoMedioX = (xMaxima + xMinima) * 0.5;
	puntoMedioY = (yMaxima + yMinima) * 0.5;
 ...
 * Ver el diagrama 1 del cuaderno
 */
void Dcel::dibujarCaras(float obx, float oby, float obz) {
	glBegin(GL_TRIANGLES);
		for(unsigned int i = 0; i < ancho - 1; i++) {
			dspFila = 0;
			for(unsigned int j = 0; j < alto - 1; j++) {
				vertActual1 = lista_v + i + dspFila;
				vertActual2 = vertActual1 + 1;
				vertActual3 = vertActual1 + ancho;
				vertActual4 = vertActual3 + 1;

				glColor3f(0.5,0.5,0.0);
				glVertex3f(vertActual1->x, vertActual1->y, vertActual1->z);
				glColor3f(0.5,0.0,0.0);
				glVertex3f(vertActual2->x, vertActual2->y, vertActual2->z);
				glColor3f(0.0,0.0,0.0);
				glVertex3f(vertActual3->x, vertActual3->y, vertActual3->z);

				glColor3f(0.0,0.5,0.0);
				glVertex3f(vertActual2->x, vertActual2->y, vertActual2->z);
				glColor3f(0.0,0.0,0.5);
				glVertex3f(vertActual4->x, vertActual4->y, vertActual4->z);
...
/*float Dcel::getCentroX() {

}

float Dcel::getCentroY() {

}

float Dcel::getCentroZ() {
